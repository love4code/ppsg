...
exports.publicIndex = async (req, res) => {
  try {
    ...
    if (mongoose.connection.readyState !== 1) {
      console.warn('⚠️  Database not connected, showing empty projects page')
      return res.render('public/projects/index', {
        projects: [],
        currentPage: 1,
        totalPages: 0
      })
    ...
    // Log if no projects found
    if (projects.length === 0 && page === 1) {
      console.warn('⚠️  Projects page: No published projects found in database')
    }
...
exports.publicShow = async (req, res) => {
  try {
    ...
    if (project.ogImage && project.ogImage._id) {
      ogImageUrl = `${req.protocol}://${req.get('host')}/admin/media/image/${
        project.ogImage._id
      }/large`
    }
